...
export function generateCityPageContent(input: CityContentInput) {
  ...
  return {
    ...
    popularSearches: [
      // Focused 20-item list — kept to highest-volume, city-specific terms only.
      // Reduced from 140+ to prevent keyword stuffing signals in rendered HTML.
      ...
      `${city.name} cab near me`,
      // Alternate name variants — only first alternate name to avoid bloat
      ...(city.alternateNames?.slice(0, 1) || []).flatMap(alt => [
        `cab service in ${alt}`,
        `${alt} to kolkata cab`,
      ]),
      // Hindi/Hinglish — 2 most-searched local terms
