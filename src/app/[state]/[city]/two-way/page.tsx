...
import { generateTwoWayServiceContent } from '@/lib/serviceContent';
...
export async function generateMetadata({ params }: { params: Promise<{ state: string; city: string }> }): Promise<Metadata> {
  ...
  const canonicalUrl = `${BUSINESS.domain}/${stateSlug}/${citySlug}/two-way`;
  return {
    title: `Two Way Cab from ${city.name} ${metaPrices.sedanPerKm} | Return Taxi with Driver | ${BUSINESS.name}`,
    description: `Two-way cab from ${city.name}. Onward + return with same driver. Sedan ${metaPrices.sedanPerKm}, SUV ${metaPrices.suvPerKm}. Min 250 km/day. 24/7. Call ${BUSINESS.phone}`.slice(0, 160),
    openGraph: { title: `Two Way Cab from ${city.name} | ${BUSINESS.name}`, type: 'website', siteName: BUSINESS.name, url: canonicalUrl, locale: 'en_IN', images: [{ url: `${BUSINESS.domain}/navbanner.webp`, width: 1200, height: 630, alt: `Two Way Cab from ${city.name}` }] },
    twitter: { card: 'summary_large_image', title: `Two Way Taxi ${city.name} | ${BUSINESS.name}`, images: [`${BUSINESS.domain}/navbanner.webp`] },
    alternates: { canonical: canonicalUrl },
    other: { thumbnail: `${BUSINESS.domain}/navbanner.webp` },
    keywords: [
      `two way cab from ${city.name}`,
      `two way taxi ${city.name}`,
      `return cab from ${city.name}`,
      `${city.name} two way cab`,
      `${city.name} return taxi`,
      `two way cab booking ${city.name}`,
      `onward return cab ${city.name}`,
      `same driver return cab ${city.name}`,
      `${city.name} to kolkata two way cab`,
      `kolkata to ${city.name} two way cab`,
      `multi day cab ${city.name}`,
      `two way cab ${metaPrices.sedanPerKm} ${city.name}`,
      `two way suv ${city.name}`,
      `two way innova ${city.name}`,
    ],
  };
}

export default async function TwoWayPage({ params }: { params: Promise<{ state: string; city: string }> }) {
  const { state: stateSlug, city: citySlug } = await params;
  ...
  const content = generateTwoWayServiceContent({ cityName: city.name, stateName: state.name, stateSlug: state.slug, citySlug: city.slug, landmarks: city.landmarks, airport: city.airport, railway: city.railway });
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateCitySubServiceSchema(city.name, state.name, state.slug, city.slug, 'Two Way Cab Service', 'two-way', `Two-way cab from ${city.name}. Onward + return with same driver. Sedan ${prices.sedanPerKm}, SUV ${prices.suvPerKm}. Min 250 km/day. 24/7.`, '12', '22')) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateFaqSchema(content.faqs)) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBreadcrumbSchema([{ name: 'Home', url: BUSINESS.domain }, { name: state.name, url: `${BUSINESS.domain}/${state.slug}` }, { name: city.name, url: `${BUSINESS.domain}/${state.slug}/${city.slug}` }, { name: 'Two Way', url: `${BUSINESS.domain}/${stateSlug}/${citySlug}/two-way` }])) }} />

      <section className="relative text-white py-14 lg:py-20 overflow-hidden">
        <HeroBanner hideDots />
        <div className="relative z-10 max-w-7xl mx-auto px-4">
          <Breadcrumbs items={[{ name: state.name, href: `/${state.slug}` }, { name: city.name, href: `/${state.slug}/${city.slug}` }, { name: 'Two Way', href: `/${state.slug}/${city.slug}/two-way` }]} />
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold mt-4 mb-4">Two Way Cab from <span className="text-gradient">{city.name}</span> {prices.sedanPerKm}</h1>
          <p className="text-lg text-gray-300 max-w-3xl mb-6">Onward + return with same driver. Cab available at destination for local travel. Sedan, SUV, Tempo. 24/7 — no surge pricing.</p>
          <div className="flex flex-wrap gap-3">
            ...
            <a href={`https://wa.me/${BUSINESS.whatsapp}?text=${encodeURIComponent(`Hi! I need a two-way cab from ${city.name}.`)}`} className="inline-flex items-center gap-2 px-6 py-3 bg-green-500 text-white font-bold rounded-full shadow-lg">💬 WhatsApp</a>
          </div>
        </div>
      </section>
      ...
      <section className="py-6 bg-white border-b border-gray-100"><div className="max-w-7xl mx-auto px-4"><div className="flex flex-wrap justify-center gap-6 text-sm">
        {[{ icon: <CheckCircle size={16} />, t: 'Same Driver Both Ways' }, { icon: <Shield size={16} />, t: 'Verified Drivers' }, { icon: <Clock size={16} />, t: '24/7 Available' }, { icon: <Star size={16} />, t: 'No Surge' }].map((b, i) => (<div key={i} className="flex items-center gap-1.5 text-primary font-medium">{b.icon} {b.t}</div>))}
      </div></div></section>
      ...
      <section className="py-12 bg-white"><div className="max-w-7xl mx-auto px-4"><div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div><h2 className="text-2xl font-bold text-secondary mb-4">Two Way Cab from {city.name}, {state.name} — Onward & Return with Same Driver</h2>
            ...
            <div className="mb-6 p-5 bg-amber-50 border border-amber-200 rounded-2xl flex flex-col md:flex-row gap-4 items-center">
              <div className="flex-1 w-full text-left">
                <h4 className="text-base font-bold text-secondary mb-3 flex items-center gap-2">
                  ⚡ Quick Answer — {city.name} Two-Way Cab Facts
                </h4>
                <dl className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                  ...
                  <div className="bg-white rounded-xl p-3 text-center border border-amber-100">
                    ...
                    <dd className="font-bold text-primary">₹300/night</dd>
                  </div>
                </dl>
                <p className="text-xs text-gray-500 mt-3">
                  📍 Ideal for: Same-day or next-day return trips where you need the same vehicle and driver to stay with you.
                </p>
                <p className="text-xs text-gray-500 mt-1">
                  📞 24/7 Helpline: <a href={`tel:${BUSINESS.phone}`} className="text-primary font-semibold hover:underline">{BUSINESS.phone}</a> | Reliable highway drivers.
                </p>
              </div>
              <div className="relative w-full md:w-40 h-28 rounded-xl overflow-hidden shrink-0 border border-amber-200 shadow-sm">
                <Image
                  src="/navbanner.webp"
                  alt={`Two way cab service from ${city.name} - booking AC sedan and SUV taxi`}
                  fill
          ...
          <div><h3 className="text-xl font-bold text-secondary mb-4">Two Way Fare from {city.name}</h3>
            ...
            <p className="text-xs text-gray-400 mt-2">* Driver allowance ₹300/night. Toll, parking extra. GST included.</p>
          </div>
          <div><h3 className="text-xl font-bold text-secondary mb-4">Why Two Way from {city.name}?</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {['Same driver for both onward & return', 'Cab available at destination for local use', 'More economical for 2+ day trips', 'No surge pricing — fixed rate 24/7', 'Driver accommodation included', 'Multi-day packages up to 30 days', 'AC, GPS-tracked vehicles', 'WhatsApp confirmation in 2 min'].map((item, i) => (<div key={i} className="flex items-start gap-2 text-sm text-gray-600"><CheckCircle size={16} className="text-green-500 shrink-0 mt-0.5" />{item}</div>))}
            </div>
          </div>
        </div>
        <div className="lg:sticky lg:top-24 lg:self-start space-y-4">
          <BookingForm defaultFrom={city.name} compact />
          <div className="p-4 bg-secondary rounded-xl text-white text-center"><p className="text-sm text-gray-300 mb-2">Two Way Helpline</p><a href={`tel:${BUSINESS.phone}`} className="text-xl font-bold hover:text-primary transition-colors">📞 {BUSINESS.phone}</a></div>
        </div>
      </div></div></section>

      {routes.length > 0 && (<section className="py-12 bg-gray-50"><div className="max-w-7xl mx-auto px-4">
        <h2 className="text-2xl font-bold text-secondary mb-6">Popular Two Way Routes from <span className="text-primary">{city.name}</span></h2>
      ...
      <section className="py-12 bg-gradient-to-r from-primary to-amber-500 text-white text-center"><div className="max-w-4xl mx-auto px-4">
        <h2 className="text-2xl md:text-3xl font-bold mb-3">Book Two Way Cab from {city.name}!</h2>
        <p className="text-white/90 mb-6">{prices.sedanPerKm} | Same Driver | Multi-Day | 24/7</p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <a href={`https://wa.me/${BUSINESS.whatsapp}?text=${encodeURIComponent(`Hi! Two-way cab from ${city.name}.`)}`} className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-green-500 text-white font-bold rounded-full text-lg shadow-lg hover:scale-105 transition-all">💬 WhatsApp</a>
        </div>
      </div></section>
    </>
  );
